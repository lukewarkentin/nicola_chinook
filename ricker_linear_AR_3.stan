...
data {
  ...
  vector[N] ice_days; // number of ice days on river
...
parameters {
  ...
  real b3; // effect of ice days - rearing
  real b4; // effect of sep-dec max flow - spawning
  real b5; // effect of mean aug flow - rearing
...
transformed parameters {
  ...
  log_R_mean1 = lnalpha + log(S) - beta * S +  b1 * ocean_surv + b2 * aug_mean_flow + b3 * ice_days + b4 * sep_dec_max_flow + b5 * aug_mean_flow_rear; // model predicting recruits from alpha and beta and covariates
   for(i in 1:N) {
