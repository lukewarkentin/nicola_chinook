...
transformed parameters {
  ...
  log_R_mean1 = lnalpha + log(SW+SH) - betaW * SW - betaH * SH + OSA * b1 + max_flow_fall * b2 + mean_flow_aug * b3; // model predicting recruits from alpha and beta and ocean survival anomaly, max winter flow, and drought index
   for(i in 1:N) {
...
generated quantities {
  ...
  vector[N] log_lik;
  for (i in 1:N) {
    pp_log_R[i] = normal_rng(log_R_mean2[i], tau); // generate predicted recruits
    pp_R[i] = exp(pp_log_R[i]);
